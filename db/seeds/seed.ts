...
async function seed() {
  // ── Idempotency guard ──────────────────────────────────────────────
  const existing = await db.select().from(schema.tenants).where(eq(schema.tenants.slug, 'tillster')).limit(1);
  if (existing.length > 0) {
    console.log('Tillster tenant already exists — skipping seed (idempotent).');
    await pool.end();
    process.exit(0);
  }
  ...
  console.log(`  jon@atkinsps.com → Tillster admin`);
  ...
  console.log('\nUsers:');
  console.log('  jon@atkinsps.com / drydock2026 → Tillster admin');
